    ...
    <div class="col-xs-12 col-sm-12 col-md-4 col-lg-3">
        <div class="cms_module_list module_list_1">
            <h3 class="menuText f-s-17">
                ...
                {!! Form::select('type',$types,$type,['class' => 'form-control select-custom-modal-type']) !!}
                </span>
            </h3>
            <hr>
                ...
                @if(count($selectMenuItems))
                    @foreach($selectMenuItems as $key=>$value)
                        <li class="@if($p==$key) active_class @endif">
                            <a data-url="{!! url('/admin/framework/main-css/get-apply-custom-classes') !!}"  href="javascript:void(0);" data-tab="#tab-2" data-href="?type={{ $type }}&amp;p={!! $key !!}" profile-id="11" main-type="text_size"
